/**
 * scan.ts — indicizza media/library/ dentro data/library.db.
 *
...
import { openDb, DATA_DIR } from '../server/db.ts';
...
async function main() {
  if (!existsSync(LIBRARY)) {
    console.error(`Nessuna libreria in ${LIBRARY}. Lancia prima: npm run seed`);
    process.exit(1);
  }
  ...
  for await (const file of walk(LIBRARY)) {
    ...
    process.stdout.write(`  ${existing ? '↻' : '+'} ${artistName} — ${albumTitle} — ${title}\n`);
  }
  ...
  console.log(
    `\nAggiunte ${added}, aggiornate ${updated}, invariate ${skipped}, rimosse ${gone.length}.` +
    `\nIn libreria: ${total} tracce.`,
  );
  db.close();
}
